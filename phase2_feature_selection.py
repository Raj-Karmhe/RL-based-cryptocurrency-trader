...
def run_feature_selection(train_feat: pd.DataFrame,
    ...
    val_close   = val_feat["Close"]   # ← FIX: use VAL not TEST to avoid leaking test-set info

    y_train_raw = build_target(train_close, k=config.FORWARD_K)
    y_val_raw   = build_target(val_close,   k=config.FORWARD_K)
    ...
    # FIX: Use validation set (not test set) to evaluate the SHAP proxy model.
    # Using the test set here would leak future return information into feature selection.
    val_aligned = val_feat[feature_cols].copy()
    val_aligned["__y"] = y_val_raw.values
    val_aligned.dropna(inplace=True)
    X_val = val_aligned.drop(columns=["__y"])
    y_val = val_aligned["__y"]

    print(f"  Train target: {len(y_train):,} rows, "
          f"mean={y_train.mean():.4f}, std={y_train.std():.4f}")
    print(f"  Val   target: {len(y_val):,} rows, "
          f"mean={y_val.mean():.4f}, std={y_val.std():.4f}")
    ...
    golden_features = stage3_shap_selection(
        X_train, y_train,
        X_val,   y_val,   # ← FIX: val set used for evaluation, not test set
...
if __name__ == "__main__":
    ...
    for symbol in symbols_to_process:
        ...
        # ── Force Include Alternative Data ──
        for c in train_feat.columns:
            if c.startswith("alt_"):
                all_selected_features.add(c)
