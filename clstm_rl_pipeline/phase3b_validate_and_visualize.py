...
5.  Saves:
    • results/interactive_validation.html  ← open in any browser
    • results/val_metrics.json
    • results/val_trades.csv
    • results/validation_summary_chart.png     ← static matplotlib fallback
...
def run_test_and_visualize(override_fee: float = None, override_slippage: float = None, model_override: str = None, coin_override: str = None):
    """
    Main Phase 3b function.
    """
    print("\n" + "=" * 70)
    print("  PHASE 3b — VALIDATION TEST & VISUALIZATION")
    print("=" * 70)
    ...
    for target_coin in coins_to_test:
        ...
        print("\n" + "─" * 70)
        print(f"  [Target Coin] Evaluating on: {target_coin}")
        print("─" * 70)
        ...
        train_df    = pd.read_csv(train_feat_path, index_col=0, parse_dates=True)
        val_df      = pd.read_csv(val_feat_path, index_col=0, parse_dates=True)
        ...
        val_prices = np.array(prices)
        bh_pv       = (val_prices / val_prices[0]) * config.INITIAL_BALANCE if len(val_prices) > 0 else np.array([config.INITIAL_BALANCE])
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                dates_pd = pd.to_datetime(dates)
                agent_rets = pd.Series(pv, index=dates_pd).pct_change().dropna()
                bh_rets = pd.Series(bh_pv, index=dates_pd).pct_change().dropna()
                
                print("\n  [VectorBT Metrics - Agent]")
                print(agent_rets.vbt.returns(freq='1h').stats().to_string())
                print("\n  [VectorBT Metrics - Buy & Hold]")
                print(bh_rets.vbt.returns(freq='1h').stats().to_string())
        except Exception as e:
            print(f"\n  [VectorBT] Error calculating vectorbt metrics: {e}")

        html_path = os.path.join(config.RESULTS_DIR, f"interactive_validation_{sym_file}.html")
        features_dict = {feat: val_df[feat].iloc[config.SEQ_LEN:config.SEQ_LEN + len(dates)].tolist() for feat in golden_features}
    ...
    if len(all_portfolio_values) > 1:
        ...
        arrs_pv = [np.array(p).flatten()[:min_len] for p in all_portfolio_values]
        arrs_bh = [np.array(p).flatten()[:min_len] for p in bh_portfolio_values]
        
        combined_pv = np.sum(arrs_pv, axis=0)
        combined_bh = np.sum(arrs_bh, axis=0)
        ...
        # ── Combined VectorBT Metrics ──────────────────────────────────────────
        try:
            import vectorbt as vbt
            import warnings
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                # Assume dates of the first asset for the combined portfolio index
                comb_dates_pd = pd.to_datetime(all_dates[0][:min_len]) if 'all_dates' in locals() and all_dates else pd.RangeIndex(min_len)
                c_agent_rets = pd.Series(combined_pv, index=comb_dates_pd).pct_change().dropna()
                c_bh_rets = pd.Series(combined_bh, index=comb_dates_pd).pct_change().dropna()
                
                print("\n  [VectorBT Combined Metrics - Agent]")
                print(c_agent_rets.vbt.returns(freq='1h').stats().to_string())
                print("\n  [VectorBT Combined Metrics - Buy & Hold]")
                print(c_bh_rets.vbt.returns(freq='1h').stats().to_string())
        except Exception as e:
            print(f"\n  [VectorBT] Error calculating combined vectorbt metrics: {e}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Phase 3b: Validate & Visualize")
    ...
    run_test_and_visualize(override_fee=args.fee, override_slippage=args.slippage, model_override=args.model, coin_override=args.coin)
