...
# ──────────────────────────────────────────────────────────────────────────────
# ENSEMBLE META-AGENT (HMM BASED)
# ──────────────────────────────────────────────────────────────────────────────

class MetaEnsembleWrapper:
    """
    Acts as a SB3 model wrapper that uses the trained Meta-Agent
    to route predictions to the underlying Bull/Bear/Crab experts.
    """
    def __init__(self, meta_agent, model_bull, model_bear, model_crab, test_df, meta_vec_normalize=None):
        self.meta_agent = meta_agent
        self.model_bull = model_bull
        self.model_bear = model_bear
        self.model_crab = model_crab
        self.test_df = test_df
        self.meta_vec_normalize = meta_vec_normalize
        
        # State tracking for meta-agent observation
        ...
        self.active_expert = 3  # Default to cash

    def predict(self, obs, deterministic=True):
        idx = min(self.current_step, len(self.test_df) - 1)
        raw_ret = self.test_df.iloc[idx].get("1d_log_return", 0.0)
        raw_vol = self.test_df.iloc[idx].get("1d_hvol_20", 0.0)
        price_now = self.test_df.iloc[idx]["Close"]
        
        self.ret_buffer.append(raw_ret)
        self.vol_buffer.append(raw_vol)
        if len(self.ret_buffer) > 24:
            self.ret_buffer.pop(0)
            self.vol_buffer.pop(0)
            
        smooth_ret = np.mean(self.ret_buffer)
        smooth_vol = np.mean(self.vol_buffer)
        
        # Calculate unrealized PnL
        if self.entry_price is not None and self.entry_price > 0:
            if self.last_position > 0.02:
                self.unrealized_pnl = (price_now - self.entry_price) / self.entry_price
            elif self.last_position < -0.02:
                self.unrealized_pnl = (self.entry_price - price_now) / self.entry_price
        else:
            self.unrealized_pnl = 0.0
            
        drawdown = (self.peak_value - self.portfolio_value) / (self.peak_value + 1e-8)
        norm_time = min(1.0, self.steps_since_switch / 100.0)
        ...
        meta_obs = np.array([
            smooth_ret, smooth_vol, self.last_position, self.unrealized_pnl, drawdown, 
            norm_time, expert_one_hot[0], expert_one_hot[1], expert_one_hot[2]
        ], dtype=np.float32)
        
        if self.meta_vec_normalize is not None:
            # VecNormalize expects shape (n_envs, obs_dim)
            meta_obs = self.meta_vec_normalize.normalize_obs(np.expand_dims(meta_obs, 0))[0]
        
        # 1. Ask Meta-Agent which expert to use
        meta_action, _ = self.meta_agent.predict(meta_obs, deterministic=True)
        ...
        # 2. Delegate to the chosen expert
        if meta_action == 0:
            expert_action, _ = self.model_bull.predict(obs, deterministic=deterministic)
        elif meta_action == 1:
            expert_action, _ = self.model_bear.predict(obs, deterministic=deterministic)
        elif meta_action == 2:
            expert_action, _ = self.model_crab.predict(obs, deterministic=deterministic)
        else:
            expert_action = np.array([0.0], dtype=np.float32)
            
        self.current_step += 1
        
        # 3. Approximate state updates for next step's observation
        action_val = float(expert_action[0])
        
        if self.last_position != 0 and self.current_step > 2:
            prev_price = self.test_df.iloc[self.current_step - 2]["Close"]
            if prev_price > 0:
                pnl_change = (price_now - prev_price) / prev_price
                if self.last_position < 0: pnl_change = -pnl_change
                self.portfolio_value *= (1 + pnl_change * abs(self.last_position))
                self.peak_value = max(self.peak_value, self.portfolio_value)
                
        if abs(action_val) < 0.02:
            self.entry_price = None
        elif abs(self.last_position) < 0.02 and abs(action_val) >= 0.02:
            self.entry_price = price_now
            
        self.last_position = action_val
...
def parse_trades(trade_log: list, dates: list, prices: list) -> pd.DataFrame:
    ...
    records = []
    for t in trade_log:
        # trade_log step starts at config.SEQ_LEN, but dates list starts from 0
...
def run_test_and_visualize(override_fee: float = None, override_slippage: float = None, model_override: str = None, coin_override: str = None, eval_set: str = "test"):
    ...
    """
    print("\n" + "=" * 70)
    print(f"  PHASE 5 — ENSEMBLE MIXTURE OF EXPERTS ({eval_set.upper()} SET)")
    print("=" * 70)
    ...
    base_model_path = model_override if model_override else config.MODEL_PATH
    if base_model_path.endswith(".zip"):
        base_model_path = base_model_path[:-4]

    # Prefer best_model checkpoints (saved by EvalCallback, survives retrains)
    # over the root-level final weights (which get overwritten on every retrain).
    def _resolve_model(regime: str) -> str:
        best_path  = os.path.join(config.MODELS_DIR, f"best_{regime}", "best_model.zip")
        final_path = f"{base_model_path}_{regime}.zip"
        if os.path.exists(best_path):
            ...
            raise FileNotFoundError(
                f"No model found for regime '{regime}'.\n"
                f"  Tried: {best_path}\n"
                f"  Tried: {final_path}\n"
                f"  Run: python phase3_train.py --regime {regime}"
            )
    ...
    # Auto-detect the correct feature list from the bull model's stored obs space.
    # All 3 regime models are always trained together on the same features,
    # so probing one is sufficient.
    _probe   = PPO.load(model_bull_path[:-4] if model_bull_path.endswith(".zip") else model_bull_path)
    _obs_dim = _probe.observation_space.shape[0]
    _n_feat  = (_obs_dim - 2) // config.SEQ_LEN
    ...
    if _n_feat in registry and os.path.exists(registry[_n_feat]):
        ...
        features_path = config.GOLDEN_FEATURES_PATH
    ...
    for target_coin in coins_to_test:
        ...
        if eval_set == "test":
            feat_path = os.path.join(config.DATA_DIR, f"{sym_file}_test_features.csv")
            if not os.path.exists(feat_path):
                print(f"  [Warning] {feat_path} not found. Skipping.")
                continue
            test_df = pd.read_csv(feat_path, index_col=0, parse_dates=True)
        else:
            # For validation, we use the actual val_features.csv
            val_feat_path = os.path.join(config.DATA_DIR, f"{sym_file}_val_features.csv")
            if not os.path.exists(val_feat_path):
                print(f"  [Warning] {val_feat_path} not found. Skipping.")
                continue
            test_df = pd.read_csv(val_feat_path, index_col=0, parse_dates=True)

        # We compute turbulence on the training portion to get the threshold, 
        # then apply it to the test_df
        ...
        train_df = full_train_df.iloc[:int(len(full_train_df) * 0.7)]
        ...
        dummy_env = DummyVecEnv([lambda: CryptoTradingEnv(
            test_df, golden_features, initial_balance=config.INITIAL_BALANCE, turb_threshold=turb_threshold, is_eval=True
        )])

        print("  [Ensemble] Loading Bull, Bear, and Crab models...")
        model_bull = PPO.load(model_bull_path[:-4], env=dummy_env)
        model_bear = PPO.load(model_bear_path[:-4], env=dummy_env)
        model_crab = PPO.load(model_crab_path[:-4], env=dummy_env)

        meta_agent_path = config.META_AGENT_PATH + ".zip"
        if not os.path.exists(meta_agent_path):
            raise FileNotFoundError(f"Meta-Agent model not found at {meta_agent_path}. Run phase5c_train_meta_agent.py first.")
            
        print("  [Ensemble] Loading RL Meta-Agent...")
        meta_agent = PPO.load(meta_agent_path)
        
        vec_norm_path = os.path.join(config.RESULTS_DIR, "meta_vec_normalize.pkl")
        if os.path.exists(vec_norm_path):
            from stable_baselines3.common.vec_env import VecNormalize
            # Just create a dummy env with the correct observation space for VecNormalize
            import gymnasium as gym
            class DummyMetaEnv(gym.Env):
                def __init__(self):
                    self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(9,), dtype=np.float32)
                    self.action_space = gym.spaces.Discrete(4)
            dummy_meta_vec = DummyVecEnv([lambda: DummyMetaEnv()])
            
            meta_vec_normalize = VecNormalize.load(vec_norm_path, dummy_meta_vec)
            meta_vec_normalize.training = False
            meta_vec_normalize.norm_reward = False
        else:
            meta_vec_normalize = None

        ensemble_model = MetaEnsembleWrapper(meta_agent, model_bull, model_bear, model_crab, test_df, meta_vec_normalize)

        results  = run_backtest(ensemble_model, test_df, golden_features, turb_threshold=turb_threshold)
        ...
        print(f"\n  {'Metric':<35} {'CLSTM-PPO':>14} {'Buy & Hold':>14}")
        print(f"  {'─'*63}")
        print(f"  {'Total Return (%)'   :<35} {metrics['total_return']*100:>14.2f} {bh_return*100:>14.2f}")
        print(f"  {'Max Drawdown (%)'   :<35} {metrics['max_drawdown']*100:>14.2f}")
        print(f"  {'Sharpe Ratio'       :<35} {metrics['sharpe_ratio']:>14.4f}")
        print(f"  {'Sortino Ratio'      :<35} {metrics['sortino_ratio']:>14.4f}")
        print(f"  {'Win Rate (%)'       :<35} {metrics['win_rate']*100:>14.2f}")
        print(f"  {'Loss Rate (%)'      :<35} {metrics.get('loss_rate', 0)*100:>14.2f}")
        print(f"  {'Final Portfolio ($)':<35} {metrics['final_portfolio']:>14,.2f} {bh_pv[-1]:>14,.2f}")
        print(f"  {'Total Trades'       :<35} {results['total_trades']:>14}")
        ...
        print(f"  [Plot] Interactive dashboard → {html_path}")
    ...
    if len(all_portfolio_values) > 1:
        ...
        print(f"  {'Total Combined Return (%)'   :<35} {combined_return*100:>14.2f} {combined_bh_return*100:>14.2f}")
        print(f"  {'Combined Max Drawdown (%)'   :<35} {combined_max_dd*100:>14.2f}")
        print(f"  {'Combined Sharpe Ratio'       :<35} {combined_sharpe:>14.4f}")
        print(f"  {'Final Combined Portfolio ($)':<35} {combined_pv[-1]:>14,.2f} {combined_bh[-1]:>14,.2f}")
        print("═" * 70)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fee", type=float, default=None,
                        help="Override default transaction fee (e.g., 0.001 for 0.1%)")
    parser.add_argument("--slippage", type=float, default=None,
                        help="Override default slippage (e.g., 0.0005 for 0.05%)")
    parser.add_argument("--model", type=str, default=None,
                        help="Path to a specific model .zip file to load")
    parser.add_argument("--coin", type=str, default=None,
                        help="Coin to evaluate on (e.g., 'ETH/USDT'). Defaults to primary coin.")
    parser.add_argument("--dataset", type=str, default="test", choices=["test", "val"],
                        help="Which dataset to evaluate on: 'test' (default out-of-sample) or 'val' (validation split of training set).")
