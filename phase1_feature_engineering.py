...
def engineer_features(df_merged: pd.DataFrame) -> pd.DataFrame:
    """
    Extracts the OHLCV columns for each timeframe from the merged DataFrame,
    computes indicators, and returns a single feature DataFrame.

    Parameters
    ----------
    df_merged : The merged multi-timeframe DataFrame (output of Phase 1 extract)

    Returns
    -------
    pd.DataFrame  containing only stationary indicator columns (no raw prices)
    """
    all_feats = []
    ...
    for tf in config.TIMEFRAMES:
        prefix = f"{tf}_"
        try:
            o = df_merged[f"{tf}_Open"]
            h = df_merged[f"{tf}_High"]
            l = df_merged[f"{tf}_Low"]
            c = df_merged[f"{tf}_Close"]
            v = df_merged[f"{tf}_Volume"]
        except KeyError:
            print(f"  [Warning] Columns for timeframe '{tf}' not found in merged df. Skipping.")
            continue

        print(f"  [Features] Computing indicators for {tf} …")
        feat_df = compute_indicators(o, h, l, c, v, prefix)
        all_feats.append(feat_df)
        print(f"    -> {feat_df.shape[1]} raw feature columns generated.")
    ...
    full_feat["Close"] = df_merged[f"{config.BASE_TF}_Close"]
    full_feat["ATR"]   = AverageTrueRange(
        high=df_merged[f"{config.BASE_TF}_High"],
        low=df_merged[f"{config.BASE_TF}_Low"],
        close=df_merged[f"{config.BASE_TF}_Close"],
...
def run_feature_engineering() -> tuple:
    ...
    for symbol in symbols_to_process:
        ...
        if not os.path.exists(merged_path):
            print(f"[Error] {merged_path} not found. Run phase1_data_extraction first.")
            continue
        ...
        df_train = merged.iloc[:te]
        df_val   = merged.iloc[te:ve]
        df_test  = merged.iloc[ve:]

        train_feat = engineer_features(df_train)
        val_feat   = engineer_features(df_val)
        test_feat  = engineer_features(df_test)

        common_cols = sorted(set(train_feat.columns) & set(val_feat.columns) & set(test_feat.columns))
        train_feat = train_feat[common_cols]
        val_feat   = val_feat[common_cols]
        test_feat  = test_feat[common_cols]
        feature_cols = [c for c in common_cols if c not in ("Close", "ATR")]
