...
def fetch_all_timeframes(symbol: str, force_redownload: bool = False) -> dict:
    ...
    # CRITICAL: When using paper date splits, we must start from the paper's
    # data start date (2018-01-01), NOT from "5 years ago" relative to today.
    # If we used the dynamic 5-year lookback in 2026, we'd start from ~2021,
    # completely missing the 2018-2021 paper training period.
    if getattr(config, "PAPER_DATE_SPLITS", False):
        paper_start = getattr(config, "PAPER_TRAIN_START", "2018-01-01")
        since_dt    = datetime.strptime(paper_start, "%Y-%m-%d").replace(tzinfo=timezone.utc)
        since_ms    = int(since_dt.timestamp() * 1000)
        print(f"  [Paper Mode] Downloading from paper start date: {paper_start}")
    else:
...
# ──────────────────────────────────────────────────────────────────────────────
# 1.5. ALTERNATIVE DATA DOWNLOADER
# ──────────────────────────────────────────────────────────────────────────────

def fetch_fear_and_greed() -> pd.DataFrame:
    """Fetches historical Fear and Greed Index from alternative.me"""
    print("  [Alt Data] Fetching Fear & Greed Index...")
    try:
        ...
        df = df[['value']].rename(columns={'value': 'fng_index'})
        return df
    except Exception as e:
        print(f"  [Alt Data] Error fetching Fear & Greed: {e}")
        return pd.DataFrame()

# ──────────────────────────────────────────────────────────────────────────────
# 2. MULTI-TIMEFRAME MERGER
# ──────────────────────────────────────────────────────────────────────────────
...
def run_extraction(force_redownload: bool = False) -> dict:
    ...
    for symbol in symbols_to_process:
        ...
        # ── Step 2.5: Merge Alternative Data ──
        if getattr(config, "USE_ALTERNATIVE_DATA", False):
            print("\n[Step 2.5] Integrating Alternative Data...")
            
            # Fear and Greed
            fng_df = fetch_fear_and_greed()
            if not fng_df.empty:
                # F&G is daily. Shift by 1 day to prevent lookahead!
                fng_df = fng_df.shift(1)
                fng_df = fng_df.reindex(merged.index, method="ffill")
                merged = merged.join(fng_df, how="left")
                merged['fng_index'] = merged['fng_index'].fillna(50.0) # neutral fallback
            else:
                merged['fng_index'] = 50.0
            
            # Local CSVs for Funding/OI/L2
            symbol_file = symbol.replace("/", "_")
            alt_path = os.path.join(config.DATA_DIR, f"{symbol_file}_alt_data.csv")
            if os.path.exists(alt_path):
                print(f"  [Alt Data] Found local alternative data: {alt_path}")
                alt_df = pd.read_csv(alt_path, index_col=0, parse_dates=True)
                # Ensure it's reindexed and forward filled onto our 1h timeline
                alt_df = alt_df.reindex(merged.index, method="ffill")
                merged = merged.join(alt_df, how="left")
                merged.fillna(0.0, inplace=True)
            else:
                print(f"  [Alt Data] No local CSV found at {alt_path}. Skipping Funding/OI/L2 data.")
                # Create dummy columns so phase1_feature_engineering doesn't crash
                merged['funding_rate'] = 0.0
                merged['open_interest'] = 0.0
                merged['l2_imbalance'] = 0.0
