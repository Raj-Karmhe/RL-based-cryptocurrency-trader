...
# Fix for Kaggle CUDA RNN/LSTM Kernel error
...
def run_training(regime: str = "all"):
    ...
    # ── Apply paper date splits if enabled ───────────────────────────────────────────────
    if getattr(config, "PAPER_DATE_SPLITS", False):
        print(f"  [Paper Dates] Applying paper train/val splits from config...")
        train_dfs = [
            df.loc[config.PAPER_TRAIN_START : config.PAPER_TRAIN_END]
            for df in train_dfs
        ]
        val_dfs = [
            df.loc[config.PAPER_VAL_START : config.PAPER_VAL_END]
            for df in val_dfs
        ]
        for i, symbol in enumerate(symbols_to_process):
            print(f"  {symbol} (after paper split) -> Train: {len(train_dfs[i]):,} rows | Val: {len(val_dfs[i]):,} rows")

        # Sanity check: ensure no overlap between train and val periods
        for i, symbol in enumerate(symbols_to_process):
            if len(train_dfs[i]) == 0:
                raise ValueError(f"[Paper Dates] Train set for {symbol} is EMPTY! "
                                 f"Check PAPER_TRAIN_START/END in config.py.")
            if len(val_dfs[i]) == 0:
                raise ValueError(f"[Paper Dates] Val set for {symbol} is EMPTY! "
                                 f"Check PAPER_VAL_START/END in config.py.")
            train_end = train_dfs[i].index.max()
            val_start = val_dfs[i].index.min()
            if val_start <= train_end:
                print(f"  [WARNING] {symbol}: Val start ({val_start.date()}) overlaps with "
                      f"Train end ({train_end.date()})! Possible data leakage.")
            else:
                print(f"  [OK] {symbol}: Clean split — Train ends {train_end.date()}, "
                      f"Val starts {val_start.date()}.")
    ...
    if torch.cuda.is_available():
        ...
        print(f"\n  [SUCCESS]  GPU detected: {gpu_name}  ({vram_gb:.1f} GB VRAM)")
        print(f"      Training will run on CUDA.")
    else:
        device = "cpu"
        print("\n  [Warning] No CUDA GPU found - training on CPU.")
    ...
    if os.path.exists(model_zip) and not config.FORCE_RETRAIN:
        ...
        if getattr(config, "RESUME_TRAINING", False):
            print(f"  Model loaded. RESUMING training for {config.TOTAL_TIMESTEPS} steps...")
            # PPO.load does not automatically update the learning rate to config
        ...
        print(f"\n  Model architecture:")
        print(f"    CLSTM: {n_feat} features x {config.SEQ_LEN} steps "
    ...
    if do_train:
        ...
        callback = TrainingMonitorCallback(log_freq=2, verbose=1)

        # Save a snapshot every CHECKPOINT_INTERVAL steps
        checkpoint_interval = getattr(config, "CHECKPOINT_INTERVAL", 0)
        callbacks_list = [callback, eval_callback]
        if checkpoint_interval > 0:
            ...
            checkpoint_cb = CheckpointCallback(
                ...
                name_prefix=f"{os.path.basename(base_model_path)}_ckpt",
                verbose=1,
            )
            callbacks_list.append(checkpoint_cb)
            print(f"  [Checkpoint] Saving every {checkpoint_interval:,} steps to {checkpoint_dir}")

        model.learn(
            total_timesteps = config.TOTAL_TIMESTEPS,
            callback        = callbacks_list,
            progress_bar    = False,   # Disabled for Kaggle to prevent log timeout
        )
    ...
    print("\n  PHASE 3 TRAINING COMPLETE [SUCCESS]")
    return model
...
def run_backtest(model: PPO,
    ...
    # Dates start from the environment's actual starting step
    ...
    while True:
        ...
        # Timing fix: append revaluation_price (bar T+1 close) so the prices[]
        # list is aligned with portfolio_values[] — both computed from the same bar.
        prices.append(info.get("revaluation_price", info["current_price"]))
